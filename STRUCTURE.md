...
policy_refs:                    # 적용되는 횡단 정책 (entity 급) 슬러그 — [[ue-cross-cutting-policies/index]] §2.1·§3
  - component-policies
...
> `policy_refs` 는 **entity 급 정책**(07 profiling / 09 global-iterator / 10 component / 11 asset-loading / 12 asset-optimization)만 담는다. ✅·△ 로 적용되는 정책의 슬러그만 나열하고, ➖(해당 없음) 정책은 넣지 않는다(본문 표에서만 ➖ 로 기록). 메타·워크플로우 정책(14~19)은 entity 에 부착하지 않는다. 적용 판정의 단일 진실원 = [[ue-cross-cutting-policies/index]] §3 매트릭스.
...
6. **횡단 정책 준수** — [[ue-cross-cutting-policies/index]] §3 매트릭스로 정한 entity 급 정책 5종을 표로 평가. 각 행 = 정책 · 적용(✅/△/➖) · 코드상 준수 근거 또는 위반·미확인 · 신뢰도(🟢/🟡/❓). 생성기가 이 클래스를 확장·수정할 때 강제할 정책의 출처. (양식·예시는 ingest 스킬 `references/cross-cutting-policies.md`.)
7. **연관 entity** — 다른 entity 링크, 미인덱싱 의존은 "(미인덱싱)" 표기.
...
- **정책 준수는 코드 근거로만**: `횡단 정책 준수` 의 준수/위반 판정은 KMCProject 코드(또는 코드 추출된 본문)를 근거로 🟢/🟡, 근거가 없으면 ❓(미확인 — 사용자 검토)로 둔다. 정책 *본문* 자체의 🔴(일반 지식) 와 혼동하지 않는다. 정책끼리 충돌하면 [[ue-cross-cutting-policies/16_PolicyPriority]] 우선순위.
- **정책·entity 둘 다 없으면 직접 코드 확인**: 적용 entity 도 없고 정책 판정도 안 서는 항목은 추측하지 말고 raw 를 직접 확인하여 근거 출처 마커로 남긴다 — 🟢 VAULT / 🔍 RAW(직접 확인, `경로:line` 증거) / 🔍❓ RAW-PENDING(못 봄) / 🔴 INFERRED. 결정 트리·마커 정의 = [[ue-cross-cutting-policies/index]] §5·§6.1.

### 4.4 양식 변경 이력 (계약 개정 — CLAUDE.md §4 절차 준수)

- **2026-05-31** — 횡단 정책 도입. frontmatter `policy_refs` 필드 + 본문 §6 `횡단 정책 준수` 섹션 신설(기존 §6 연관 entity → §7). 단일 진실원 = [[ue-cross-cutting-policies/index]]. ingest-kmc 스킬(SKILL.md/entity-template.md/cross-cutting-policies.md) 동기화. 기존 entity 는 단위별 마이그레이션(Component 카테고리 파일럿 선행).
