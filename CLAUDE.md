...
**4. 정책 적용 (entity 를 찾았으면 반드시)** — 해당 entity 의 `policy_refs` + 본문 §6 `횡단 정책 준수` 를 읽어, 이 클래스를 수정·확장할 때 **강제할 횡단 정책**을 plan 에 싣는다. entity 가 없어(2·3차) 새 클래스를 만들 땐 [[ue-cross-cutting-policies/index]] §3 매트릭스로 카테고리에 걸리는 정책을 직접 적용. 정책끼리 충돌하면 [[ue-cross-cutting-policies/16_PolicyPriority]] 우선순위로 해소.

**5. 정책·entity 둘 다 없는 케이스 — 직접 코드 확인(raw fallback) 의무** — entity 페이지도 없고 §3 매트릭스로 정책 판정도 안 서면 **KB 만으로 답하지 말고 raw(KMCProject 코드)를 직접 grep·읽기로 확인**한다. 확인 결과는 [[ue-cross-cutting-policies/index]] §5·§6.1 의 근거 출처 마커로 남긴다 — 🟢 VAULT(entity 근거) / 🔍 RAW(raw 직접 확인 완료, `경로:line` 증거 의무) / 🔍❓ RAW-PENDING(raw 미마운트 등 확인 못 함 — 사용자 검토) / 🔴 INFERRED(raw 에도 선례 없어 일반 지식). "정책도 entity 도 없다"는 *모른다*가 아니라 *raw 를 봐야 한다*는 신호다 — 추측으로 메우지 않는다.

locate-report 에는 각 항목의 출처 태그(🟢/🔍/🔍❓/🔴) + **적용 정책(과 그 entity 의 ❌ 위반·❓ 미확인 항목)** 을 반드시 표기. plan 체크포인트에서 🔴·🔍❓ 와 정책 ❌/❓ 가 집중 검토 대상.

> **세 인덱스의 역할 분담** — `index.md`(entity 진입점) → 무엇이 있나. `categories.md`(분류 SSOT) → 어디에 속하나. [[ue-cross-cutting-policies/index]](정책 SSOT) → 어떤 공통 규약을 지켜야 하나. ingest 는 셋을 모두 쓰며(쓰기), locate 는 1차로 `index.md`→entity 를 찾고 그 entity 의 `policy_refs` 를 통해 정책 인덱스를 **간접 참조**한다(읽기).
